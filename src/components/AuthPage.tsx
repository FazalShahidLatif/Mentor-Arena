...
const GOOGLE_CLIENT_ID =
  ...
  '524446216074-121be4jq4eloq5akpmskk1a83gkfbjp6.apps.googleusercontent.com';
...
export const AuthPage: React.FC<AuthPageProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const tokenClient = (window as any).google.accounts.oauth2.initTokenClient({
        ...
        callback: async (tokenResponse: any) => {
          if (tokenResponse && tokenResponse.access_token) {
            try {
              setSuccessMessage('Verifying Google credentials with Mentor Arena...');
              const userInfoRes = await fetch('https://www.googleapis.com/oauth2/v3/userinfo', {
                headers: { Authorization: `Bearer ${tokenResponse.access_token}` },
              });
              const profile = await userInfoRes.json();

              // Sync authenticated user with server
              try {
                await fetch('/api/auth/google', {
                  ...
                  body: JSON.stringify({ profile, token: tokenResponse.access_token }),
                });
              } catch (err) {
                console.warn('Server user sync note:', err);
              }
              ...
              setLoading(false);
              setSuccessMessage(`Welcome, ${profile.name || profile.email}! Redirecting...`);
              setTimeout(() => onBackToHome(), 600);
            } catch (fetchErr: any) {
              setLoading(false);
              setErrorMessage(fetchErr.message || 'Failed to retrieve profile from Google');
            }
          } else if (tokenResponse?.error) {
            setLoading(false);
            setErrorMessage(tokenResponse.error_description || tokenResponse.error || 'Google login cancelled');
          }
        },
        error_callback: (err: any) => {
          setLoading(false);
          console.warn('Google Auth error:', err);
          setErrorMessage('Google Sign-In was closed or interrupted.');
        },
      });
      ...
      setLoading(false);
      console.error('Google Auth Init Error:', initErr);
